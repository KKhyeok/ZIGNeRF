...
lr_i = cfg['training']['learning_rate_i']
...
def weight_init_kaiming_uniform(net):
...
model.generator.apply(weight_init_kaiming_uniform)
model.discriminator.apply(weight_init_kaiming_uniform)
model.identifier.apply(weight_init_kaiming_uniform)
...
if hasattr(model, "identifier") and model.identifier is not None:
    parameters_i = model.identifier.parameters()
    optimizer_i = op(parameters_i, lr=lr_i)
else:
    optimizer_i = None

trainer = config.get_trainer(model, optimizer, optimizer_d, optimizer_i, cfg, device=device) 
checkpoint_io = CheckpointIO(out_dir, model=model, optimizer=optimizer,
                             optimizer_d=optimizer_d, optimizer_i=optimizer_i)
try:
...
inv_start_iter = cfg['training']['inv_start_iter']
...
if hasattr(model, "identifier") and model.identifier is not None:
    nparameters_i = sum(p.numel() for p in model.identifier.parameters())
    logger_py.info('Total number of identifier parameters: %d' % nparameters_i)
t0b = time.time()
...
while (True):
    ...
    for batch in train_loader:
        ...
        # Visualize cond image
        if vis_cond_flag == 0:
            logger_py.info('Visualizing')
            image_grid = trainer.visualize_cond(it=it)
            if image_grid is not None:
                logger.add_image('cond_images', image_grid, it)
            vis_cond_flag =1
                        
        # # Visualize output
        if visualize_every > 0 and (it % visualize_every) == 0 and it <= inv_start_iter:  
            logger_py.info('Visualizing')
        ...
        # # Visualize output
        if visualize_every > 0 and (it % visualize_every) == 0 and it > inv_start_iter: 
            logger_py.info('Visualizing_inversion')
            image_grid = trainer.visualize_inversion(it=it)
            if image_grid is not None:
                logger.add_image('images_inversion', image_grid, it)
