...
from im2scene.discriminator.conv import Identifier
...
def get_model(cfg, device=None, len_dataset=0, **kwargs):
    ...
    render_batch_size = cfg['rendering']['render_batch_size']
    # Load always the decoder
    decoder = models.decoder_dict[decoder](
        z_dim=z_dim, img_size=img_size, **decoder_kwargs
    )
    ...
    if generator is not None:
        generator = models.generator_dict[generator](
            ...
            neural_renderer=neural_renderer, img_size=img_size, render_batch_size= render_batch_size, cfg=cfg, **generator_kwargs)
    identifier = Identifier(img_size=img_size , z_dim=z_dim, z_dim_bg=z_dim_bg, batch_size=batch_size)
    ...
    model = models.GIRAFFE(
        ...
        generator_test=generator_test, identifier = identifier
    )
    return model

def get_trainer(model, optimizer, optimizer_d, optimizer_i, cfg, device, **kwargs):
    ...
    trainer = training.Trainer(
        model, optimizer, optimizer_d, optimizer_i, device=device, vis_dir=vis_dir,
        overwrite_visualization=overwrite_visualization, multi_gpu=multi_gpu,
        fid_dict=fid_dict,
        n_eval_iterations=n_eval_iterations, inv_start_iter = inv_start_iter
    )
