...
from im2scene.utils import ImgEncoder
...
class Generator(nn.Module):
    ...
    def __init__(self, device, z_dim=256, z_dim_bg=128, decoder=None,
                 ...
                 cond=True, img_size=None, render_batch_size = None, cfg=None, **kwargs):
        super().__init__()
        self.device = device
        ...
        self.train_dataset = config.get_dataset(cfg)
        self.train_loader = torch.utils.data.DataLoader(self.train_dataset, batch_size=8, shuffle=False, pin_memory=True, drop_last=True)
        self.render_loader = torch.utils.data.DataLoader(self.train_dataset, batch_size=self.render_batch_size, shuffle=False, pin_memory=True, drop_last=True)
    ...
    def forward(self, batch_size=32, latent_codes=None, camera_matrices=None,
        ...
        if return_alpha_map:
            ...
            rgb_v = self.volume_render_image(
                ...
                only_render_background=only_render_background, cond_data=cond_data, reconstruct = reconstruct)
            ...
            if get_info == False:
                ...
                return rgb, camera_matrices, transformations, bg_rotation
    ...
    def get_latent_codes(self, batch_size=32, tmp=1.):
        z_dim, z_dim_bg = self.z_dim, self.z_dim_bg     #z_dim = 256, z_dim_bg = 128
    ...
    def get_vis_dict(self, batch_size=32):
        ...
        cond_data = data['cond'].repeat(int(batch_size/8), 1 ,1, 1).to(self.device)
        latent_codes = self.get_latent_codes(8)
        latent_codes = (
            latent_codes[0].repeat(8,1,1),
            latent_codes[1].repeat(8,1,1),
            latent_codes[2].repeat(8,1),
            latent_codes[3].repeat(8,1)
        )
        vis_dict = {
    ...
    def get_train_dict(self, batch_size=32):
        vis_dict = {
            'batch_size': batch_size,
            'latent_codes': self.get_latent_codes(batch_size),
            'camera_matrices': self.get_random_camera(batch_size),
    ...
    def get_optimize_dict(self, batch_size=32):
        vis_dict = {
            'latent_codes': self.get_latent_codes(batch_size),
            'camera_matrices': self.get_random_camera(batch_size),
            'transformations': self.get_random_transformations(batch_size),
    ...
    def transform_points_to_box(self, p, transformations, box_idx=0,
                                reconstruct = False, scale_factor=1.):
    ...
    def get_evaluation_points(self, pixels_world, camera_world, di,
                              transformations, i, reconstruct = False):
        batch_size = pixels_world.shape[0]
        n_steps = di.shape[-1] 

        pixels_world_i = self.transform_points_to_box(
            pixels_world, transformations, i,reconstruct)
        camera_world_i = self.transform_points_to_box(
            camera_world, transformations, i,reconstruct)
    ...
    def volume_render_image(self, latent_codes, camera_matrices,
                            ...
                            not_render_background=False,
                            only_render_background=False, cond_data=None, reconstruct = False):
        ...
        for i in range(n_iter):             
            if i < n_boxes:  # Object                
                p_i, r_i = self.get_evaluation_points(      
                    pixels_world, camera_world, di, transformations, i , reconstruct)
